...
class ccm (read_fits):
        ...
        def cal_ccm_einsum(self):
                ...
                # 'ati,bti->iab' calculates the dot product over the 't' (middle) axis 
                # for each 'i' (channel), returning a stack of (channel, beam, beam) matrices.
                dask_ccm = da.einsum('ati,bti->iab', dask_array, dask_array)
                
                # Execute the graph and return the result as a numpy array
                self.ccm = np.asarray(dask_ccm.compute(n_workers=8, threads_per_worker=1, processes=True), dtype=np.float64)
