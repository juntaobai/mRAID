...
class eig():
        def __init__(self, filenames, hdf5_prefix, nsub, sub_step, nbeam, sigma_eigval=3, sigma_eigvec=1, ignorchan_start=0, ignorchan_end=0):
                ...
                for start in range(0, nsub, self.sub_step):
                        end = start + 1 if self.sub_step == 1 else min(start + self.sub_step - 1, nsub - 1)
                ...
                logger.info(f"Combined eigval shape: {self.eigval_array.shape}")
                logger.info(f"Combined eigvec shape: {self.eigvec_array.shape}")

                self.plot_prefix = self.hdf5_prefix
        ...
        def generate_filter (self):
                ...
                # reject extremely large eigval so that we can estimate the noise level
                ...
                #logger.debug (mea, st, cou)
                #logger.debug (bounds_par)
        ...
        def cal_evc_Threshold(self):
                ...
                for i in range(0, self.nbeam, 1): 
                        ...
                        #logger.debug (np.amax(mid), np.amin(mid))
                        
                        # reject extremely large vectors close to +-1
                        index = np.where((mid > -0.8) & (mid < 0.8))
                        data = mid[index].compressed() 
                        #logger.debug (np.amax(data), np.amin(data))

                        plt.clf()
                        ax = plt.subplot(111)
                        ax.hist(data, 200)
                        plt.savefig('eigvec_hist_%d.png'%i)
        ...
        def generate_mask (self):
                ...
                if (self.ignorchan_start != 0 and self.ignorchan_end != 0 and self.ignorchan_start < self.ignorchan_end):
                        logger.info(f"Ignoring channel range [{self.ignorchan_start}:{self.ignorchan_end}]")
...
if __name__ == "__main__":
        ...
        parser.add_argument('-h5_prefix',  '--input_hdf5_prefix', metavar='Input HDF5 file prefix',          default='mRAID_test',          help='Input HDF5 file prefix')
        ...
        hdf5_prefix     = args.input_hdf5_prefix
